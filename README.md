...
Zier Alpha ships with optimized build profiles to keep binary size manageable and performance high.

### Development & Testing

- `cargo test` uses an optimized test profile (`opt-level=3`, no debug info) by default (see `.cargo/config.toml`). Tests compile quickly and run faster without debug overhead.
- `cargo build` (debug) remains fast for iteration.

### Release Builds

- `cargo build --release` enables full LTO, single codegen unit, and no debug symbols – a good balance of size and debuggability (panic backtraces preserved).

### Ultra‑Packed Distribution Build (Optional)

For distribution where binary size is critical, we provide a special profile and script that applies aggressive optimizations:

- **Profiles**:
  - `release-packed` (inherits release + `panic = "abort"` – no unwinding, ~200–500 KB smaller)
- **Script**: `scripts/build_release_upx_packed.sh`

This script:
1. Builds with `--profile release-packed` (full LTO + panic=abort).
2. Strips all possible symbols (`strip` or `-x` on macOS).
3. Compresses with UPX using LZMA (`--best`), yielding an additional 40–60% reduction.
4. Prints detailed metrics (original, stripped, UPX sizes, ratios).

**Usage**:

```bash
# One‑step build: strip + UPX compression
./scripts/build_release_upx_packed.sh

# Skip UPX (just strip)
./scripts/build_release_upx_packed.sh --no-upx

# Force rebuild even if binary is up‑to‑date
...
- Cannot produce backtraces on panic (aborts immediately).
- May take a few extra seconds to start due to UPX decompression (once per run; cached by OS).
- Triggers more antivirus/EDR false positives due to packing.

Keep a non‑packed release build for debugging; use the packed version for distribution.
