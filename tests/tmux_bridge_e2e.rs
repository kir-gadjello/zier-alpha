...
async fn test_tmux_bridge_lifecycle() {
    ...
    let policy = SandboxPolicy {
        ...
        allow_env: false,
        enable_os_sandbox: false, // Disable OS sandboxing for tests in unprivileged env
    };
    ...
    service
        .load_script(plugin_path.to_str().unwrap())
        .await
    ...
    let result = service
        .execute_tool("tmux_spawn", &spawn_args)
        .await
        .unwrap();
    ...
    let inspect = service
        .execute_tool("tmux_inspect", &inspect_args)
        .await
        .unwrap();
    ...
    let write = service
        .execute_tool("tmux_control", &write_args)
        .await
        .unwrap();
    ...
    let history = service
        .execute_tool("tmux_history", &history_args)
        .await
        .unwrap();
    ...
    let kill = service
        .execute_tool("tmux_control", &kill_args)
        .await
        .unwrap();
    ...
    let monitor = service
        .execute_tool("tmux_monitor", &monitor_args)
        .await
        .unwrap();
    ...
    if !rm_xml.contains("safety policy") {
        println!(
            "WARNING: Error message did not contain 'safety policy', got: {}",
            rm_xml
        );
    ...
    std::fs::write(
        &script_path,
        "import time; time.sleep(1); print('Enter password:', flush=True); time.sleep(5)",
    )
    .unwrap();
    ...
    let spawn_res = service
        .execute_tool("tmux_spawn", &expect_server_args)
        .await
        .unwrap();
    ...
    let expect_res = service
        .execute_tool("tmux_expect", &expect_args)
        .await
        .unwrap();
    ...
    .to_string();
    service
        .execute_tool("tmux_control", &kill_interactive)
        .await
        .unwrap();
}
