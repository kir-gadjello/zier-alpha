...
async fn test_workdir_overlay_strategy() {
    ...
    let memory =
        MemoryManager::new_with_full_config(&config.memory, Some(&config), "test").unwrap();
    ...
    agent
        .chat("test_tool:write_file|MEMORY.md|Cognitive Content")
        .await
        .unwrap();
    ...
    agent
        .chat("test_tool:write_file|main.rs|Project Content")
        .await
        .unwrap();
...
async fn test_workdir_mount_strategy() {
    ...
    agent
        .chat("test_tool:write_file|project/main.rs|Project Content")
        .await
        .unwrap();
    assert!(project_dir.join("main.rs").exists());
    assert_eq!(
...
async fn test_deno_tool_routing() {
    ...
    agent
        .chat("test_tool:test_write|MEMORY.md|Deno Cognitive Content")
        .await
        .unwrap();
    assert!(workspace_dir.join("MEMORY.md").exists());
    assert_eq!(
        fs::read_to_string(workspace_dir.join("MEMORY.md")).unwrap(),
        "Deno Cognitive Content"
    );

    // 2. Write to app.js via Deno (should go to project_dir)
    agent
        .chat("test_tool:test_write|app.js|Deno Project Content")
        .await
        .unwrap();
    assert!(project_dir.join("app.js").exists());
    assert_eq!(
        fs::read_to_string(project_dir.join("app.js")).unwrap(),
        "Deno Project Content"
    );
}
