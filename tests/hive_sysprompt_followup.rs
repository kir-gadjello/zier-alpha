...
#[test]
fn test_hive_sysprompt_followup() -> Result<()> {
    ...
    // Config with clone_sysprompt_followup
...
clone_sysprompt_followup = "You are a clone."
...
"#,
    ...
    assert!(
        stdout.contains("You are a clone."),
        "Expected system prompt to include follow-up text 'You are a clone.'. stdout: {}",
        stdout
    );
