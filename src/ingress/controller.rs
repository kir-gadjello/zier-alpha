...
pub async fn ingress_loop(
    ...
    // Shared reference to jobs and script_tools for cloning into tasks
    let jobs = Arc::new(jobs);
    let script_tools = Arc::new(script_tools);
    ...
    while let Some(msg) = rx.recv().await {
        ...
        tokio::spawn(async move {
            ...
            // Create Agent from prototype and set strategy/session
            ...
            match msg.trust {
                TrustLevel::OwnerCommand => {
                    // Update tools (rebuild if needed for dynamic updates, or just use base).
                    // We rebuild here to ensure any changed script tools are picked up,
                    // matching original behavior but in parallel.
                    match ToolRegistry::build(
                        &config,
                        Some(memory.clone()),
                        script_tools.to_vec(), // Deref Arc -> clone Vec
                        project_dir.clone(),
                    ) {
